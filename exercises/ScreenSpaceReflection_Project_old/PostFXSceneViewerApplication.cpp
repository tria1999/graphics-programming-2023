...
void PostFXSceneViewerApplication::InitializeModels()
{
    ...
    m_skyboxTexture = TextureCubemapLoader::LoadTextureShared("models/plant/StandardCubeMap_1.hdr", TextureObject::FormatRGB, TextureObject::InternalFormatRGB16F);
    ...
    /*loader.SetCreateMaterials(false);
    std::shared_ptr<Model> plantModel = loader.LoadShared("models/plant/hoewa_Forsteriana_2.obj");
    std::shared_ptr<SceneModel> m_plantModel = std::make_shared<SceneModel>("plant", plantModel);
    m_plantModel->GetTransform()->SetScale(glm::vec3(0.01));
    m_plantModel->GetTransform()->SetTranslation(glm::vec3(0.f,-1.f,0.f));
    m_scene.AddSceneNode(m_plantModel);

    std::shared_ptr<Model> buildingModel = loader.LoadShared("models/building/house.obj");
    std::shared_ptr<SceneModel> m_buildingModel = std::make_shared<SceneModel>("building", buildingModel);
    m_buildingModel->GetTransform()->SetScale(glm::vec3(0.5));
    m_buildingModel->GetTransform()->SetTranslation(glm::vec3(0.f, -1.f, 0.f));
    m_scene.AddSceneNode(m_buildingModel);*/ 
