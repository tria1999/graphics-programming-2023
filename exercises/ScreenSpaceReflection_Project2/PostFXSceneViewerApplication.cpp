...
void PostFXSceneViewerApplication::InitializeMaterials()
{
    ...
    //Water material (with SSR)
    {
        std::vector<const char*> vertexShaderPaths;
        vertexShaderPaths.push_back("shaders/version330.glsl");
        vertexShaderPaths.push_back("shaders/SSR_let.vert");
        Shader vertexShader = ShaderLoader(Shader::VertexShader).Load(vertexShaderPaths);
        ...
        fragmentShaderPaths.push_back("shaders/lambert-ggx.glsl");
        fragmentShaderPaths.push_back("shaders/lighting.glsl");
        fragmentShaderPaths.push_back("shaders/SSR_let.frag");
        Shader fragmentShader = ShaderLoader(Shader::FragmentShader).Load(fragmentShaderPaths);
        ...
        m_renderer.RegisterShaderProgram(shaderProgramPtr,
            [=](const ShaderProgram& shaderProgram, const glm::mat4& worldMatrix, const Camera& camera, bool cameraChanged)
            {
                ...
                shaderProgram.SetUniform(worldViewProjMatrixLocation, camera.GetViewProjectionMatrix() * worldMatrix);
                shaderProgram.SetUniform(timeLocation, (float)glfwGetTime());
            },
            m_renderer.GetDefaultUpdateLightsFunction(*shaderProgramPtr)
                );
...
void PostFXSceneViewerApplication::InitializeModels()
{
    ...
    // Link material properties to uniforms
    Texture2DLoader waterTextureLoader(TextureObject::FormatRGBA, TextureObject::InternalFormatRGBA8);
    ...
    waterLoader.SetMaterialProperty(ModelLoader::MaterialProperty::SpecularTexture, "SpecularTexture");

    std::shared_ptr<Model> waterModel = waterLoader.LoadShared("models/water/water.obj");
    //waterModel->GetMaterial(0).SetUniformValue("ColorTexture", waterTextureLoader.LoadShared("models/water/water.png"));
    m_scene.AddSceneNode(std::make_shared<SceneModel>("water", waterModel));

}
/*
void PostFXSceneViewerApplication::CreateWaterMaterial(std::shared_ptr<ShaderProgram> shaderProgram2, ShaderUniformCollection::NameSet filteredUniforms)
{
    ...
    filteredUniforms.insert("InvProjMatrix");
    //filteredUniforms.insert("PositionTexture");
    ...
    m_fbo->Bind();
    m_fbo->SetTexture(FramebufferObject::Target::Both, FramebufferObject::Attachment::Color0, *m_positionTexture);
    //glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, positionTexture, 0);
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
        std::cerr << "Error: Framebuffer is not complete." << std::endl;
    }

    m_waterMaterial->SetUniformValue("PositionTexture", m_positionTexture);
    m_fbo->Unbind();
    m_positionTexture->Unbind();


    std::unique_ptr<GBufferRenderPass> gbufferRenderPass(std::make_unique<GBufferRenderPass>(1024, 1024));

    // Set the g-buffer textures as properties of the deferred material
    m_waterMaterial->SetUniformValue("DepthTexture", gbufferRenderPass->GetDepthTexture());
    m_waterMaterial->SetUniformValue("NormalTexture", gbufferRenderPass->GetNormalTexture());
    ...
    //ShaderProgram::Location positionTextureLocation = shaderProgram->GetUniformLocation("PositionTexture");

    m_waterMaterial->SetShaderSetupFunction([=](ShaderProgram& shaderProgram)
        {
            shaderProgram.SetUniform(worldMatrixLocation, glm::translate(glm::vec3(0.f, 2.f, 0.f)) * glm::scale(glm::vec3(5.f)));
    ...
    //shaderProgram.SetUniform(positionTextureLocation, &m_positionTexture);
        });
}*/
