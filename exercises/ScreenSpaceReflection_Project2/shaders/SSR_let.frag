...
void SSR()
{
    ...
    vec2 texSize  = textureSize(PositionTexture, 0).xy; //this is supposed to be the position texture, look back later
  ...
  FragColor = vec4(positionFrom);

  if(texture(PositionTexture, texCoord)==vec4(0.f,0.f,0.f,1.f))
